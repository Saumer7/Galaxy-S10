...
(
    ...
    if [ $MODEL = "G970F" ]; then
    ./build-aosp mkimg model=G970F name="$K_NAME-$K_BASE-AOSP-$MODEL-$K_VERSION" toolchain=cruel +dtb
    elif [ $MODEL = "G970N" ]; then
    ./build-aosp mkimg model=G970N name="$K_NAME-$K_BASE-AOSP-$MODEL-$K_VERSION" toolchain=cruel +dtb
    elif [ $MODEL = "G973F" ]; then
    ./build-aosp mkimg model=G973F name="$K_NAME-$K_BASE-AOSP-$MODEL-$K_VERSION" toolchain=cruel +dtb
    elif [ $MODEL = "G973N" ]; then
    ./build-aosp mkimg model=G973N name="$K_NAME-$K_BASE-AOSP-$MODEL-$K_VERSION" toolchain=cruel +dtb
    elif [ $MODEL = "G975F" ]; then
    ./build-aosp mkimg model=G975F name="$K_NAME-$K_BASE-AOSP-$MODEL-$K_VERSION" toolchain=cruel +dtb
    elif [ $MODEL = "G975N" ]; then
    ./build-aosp mkimg model=G975N name="$K_NAME-$K_BASE-AOSP-$MODEL-$K_VERSION" toolchain=cruel +dtb
    elif [ $MODEL = "G977B" ]; then
    ./build-aosp mkimg model=G977B name="$K_NAME-$K_BASE-AOSP-$MODEL-$K_VERSION" toolchain=cruel +dtb
    elif [ $MODEL = "G977N" ]; then
    ./build-aosp mkimg model=G977N name="$K_NAME-$K_BASE-AOSP-$MODEL-$K_VERSION" toolchain=cruel +dtb
    elif [ $MODEL = "N970F" ]; then
    ./build-aosp mkimg model=N970F name="$K_NAME-$K_BASE-AOSP-$MODEL-$K_VERSION" toolchain=cruel +dtb
    elif [ $MODEL = "N971N" ]; then
    ./build-aosp mkimg model=N971N name="$K_NAME-$K_BASE-AOSP-$MODEL-$K_VERSION" toolchain=cruel +dtb
    elif [ $MODEL = "N975F" ]; then
    ./build-aosp mkimg model=N975F name="$K_NAME-$K_BASE-AOSP-$MODEL-$K_VERSION" toolchain=cruel +dtb
    elif [ $MODEL = "N976N" ]; then
    ./build-aosp mkimg model=N976N name="$K_NAME-$K_BASE-AOSP-$MODEL-$K_VERSION" toolchain=cruel +dtb
    elif [ $MODEL = "N976B" ]; then
    ./build-aosp mkimg model=N976B name="$K_NAME-$K_BASE-AOSP-$MODEL-$K_VERSION" toolchain=cruel +dtb
    fi
...
if [ $prompt = "1" ]; then
    MODEL=G970F
    ZIP_DATE=`date +%Y%m%d`
    ZIP_NAME=$K_NAME-$MODEL-$K_VERSION-CR-$ZIP_DATE.zip
    ...
    ZIP_DATE=`date +%Y%m%d`
    ZIP_NAME=$K_NAME-S10-N10-$K_VERSION-CR-$ZIP_DATE.zip
    export KERNEL_VERSION="$K_NAME-$K_BASE-AOSP-$MODEL-$K_VERSION"
